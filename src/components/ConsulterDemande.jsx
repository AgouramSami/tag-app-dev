...
const ConsulterDemande = ({ demande, onRetour, isJuriste = false }) => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const response = await fetch(`${API_URL}/demandes/${demande._id}`, {
          credentials: "include",
        });
  ...
  const fetchCommune = async () => {
    try {
      const response = await fetch(`${API_URL}/communes/${demande.commune}`, {
        credentials: "include",
      });
      if (response.ok) {
      ...
      // Erreur silencieuse - l'interface affichera "Chargement..."
    }
  };

  const fetchTheme = async () => {
    try {
      console.log("ID du thème:", demande.theme);
      const response = await fetch(`${API_URL}/themes/${demande.theme}`, {
        credentials: "include",
      });
      console.log("Réponse du serveur:", response.status);
      if (response.ok) {
        const themeData = await response.json();
        console.log("Données du thème:", themeData);
        setTheme(themeData);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/demandes/${demande._id}/message`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
          ...
          <div className="tag-consulter-detail-info">
            ...
            <div className="tag-consulter-info-grid">
              <div className="tag-consulter-info-item">
                <label>Thème</label>
                <span>{theme?.nom || "Chargement..."}</span>
              </div>
              <div className="tag-consulter-info-item">
                <label>Commune</label>
